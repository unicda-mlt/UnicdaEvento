...
sealed class StudentFlowRoute(override val route: String, override val title: String) : AppRouteInfo {
    ...
    data object DISCOVER_EVENT : StudentFlowRoute("student_flow_discover_event", "Discover")

    data object EVENT_DETAIL : StudentFlowRoute("student_flow_event_detail/{eventId}", "Event Details") {
        fun create(eventId: String) = "student_flow_event_detail/$eventId"
    }

    data object MY_EVENT : StudentFlowRoute("student_flow_my_event", "My Events")
    data object SETTING : StudentFlowRoute("student_flow_setting", "Setting")
